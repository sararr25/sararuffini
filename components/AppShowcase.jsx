  ...
  return (
    <div className="app-showcase-page">
      ...
      <main>
        <section className="app-showcase-hero">
          <div className="app-showcase-wrap app-showcase-hero__grid">
            <div className="app-showcase-hero__copy app-showcase-reveal">
              ...
              <div className="app-showcase-hero__actions"><a className="app-showcase-button app-showcase-button--primary" href="#development">Explore the development<span className="material-symbols-outlined">south</span></a><a className="app-showcase-button app-showcase-button--secondary" href="/pages/projects">Back to projects<span className="material-symbols-outlined">arrow_forward</span></a></div>
              <ul className="app-showcase-benefits" aria-label="Case study focus">{(content.hero.benefits || FALLBACK.hero.benefits).map((benefit) => <li key={benefit}><span className="material-symbols-outlined">check</span>{benefit}</li>)}</ul>
        ...
        <section className="app-showcase-tech" id="development"><div className="app-showcase-wrap"><p className="app-showcase-kicker">{content.tech_stack.eyebrow || "Development & implementation"}</p><h2>{content.tech_stack.title}<span className="material-symbols-outlined">code</span></h2><p className="app-showcase-section-lead">{content.tech_stack.intro}</p><div className="app-showcase-tech__grid">{techCards.map((card) => <article className={`app-showcase-tech-card app-showcase-tech-card--${card.tone || "paper"} app-showcase-tech-card--${card.tilt || "none"} app-showcase-reveal`} key={card.title}><div><span className="material-symbols-outlined">{card.icon}</span><h3>{card.title}</h3></div><p>{card.text}</p><ul>{(card.tags || []).map((tag) => <li key={tag}>{tag}</li>)}</ul></article>)}</div></div></section>
        ...
        <section className="app-showcase-action">
          ...
          <div className="app-showcase-action__cta"><a href={content.action.cta_href}>{content.action.cta_label}<span className="material-symbols-outlined">arrow_forward</span></a></div>
        </section>
      </main>
      <SiteFooter />
    </div>
