...
const ROUTES = [
  ...
  { route: "/pages/portfolio-main/code.html", file: "pages/portfolio-main/code.html" },
...
function readLegacyPage(entry) {
  ...
  html = replaceChrome(html, "nav");
  html = replaceChrome(html, "footer");
  ...
  html = html.replace(/<script\b([^>]*)>([\s\S]*?)<\/script>/gi, (full, attrs, inlineCode) => {
    ...
    if (src && /(?:shared-site-chrome|mobile-nav)\.js/i.test(src)) {
      return "";
    }
...
function replaceChrome(html, type) {
  const className = type === "nav" ? "shared-site-nav" : "shared-site-footer";
  const re = new RegExp(`<${type}\\b(?=[^>]*\\b${className}\\b)[\\s\\S]*?<\\/${type}>`, "gi");
  return html.replace(re, `<!--NEXT_CHROME_${type.toUpperCase()}-->`);
}
...
function normalizeUrl(value, file) {
  ...
  const basePath = `/${path.posix.dirname(file).replace(/^\.$/, "")}/`;
